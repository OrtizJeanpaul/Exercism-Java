public class Secrets {
    public static int shiftBack(int value, int amount) {
        throw new UnsupportedOperationException("Please implement the (static) Secrets.shiftBack() method");
    }

    public static int setBits(int value, int mask) {
        throw new UnsupportedOperationException(" implement the (static) Secrets.setBits()");
    }

    public static int flipBits(int value, int mask) {
        throw new UnsupportedOperationException(" implement the Secrets.flipBits() method");
    }

    public static int clearBits(int value, int mask) {
        throw new UnsupportedOperationException("Please implement the Secrets.clearBits() method");
    }
}